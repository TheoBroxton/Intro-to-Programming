# -*- coding: utf-8 -*-
"""
PHYS20161 Week 8 Quiz 1D uncertainty analysis 1

This code performs a 1D minimised chi square fit on data that describes a
polynomial.

Write some code to read in chi_square_analysis_data_1_a.txt to begin with and
perform the fit. Then, write some code to find the standard devation on the
coefficient.

You will need to modify the code further to work with
chi_square_analysis_data_1_b.txt and chi_square_analysis_data_1_c.txt.
(Some issues might be hard-coded here!)

Lloyd Cawthorne 24/08/21
"""
...
FILE_NAME = 'chi_square_analysis_data_1_a.txt'
...
def poly_function(x, coefficient):
    """
    Polynomial function to be fitted
    Parameters
    ----------
    x : float
...
def plot_result(data, result):
    ...
    ax.plot(data[:, 0], poly_function(data[:, 0], result))
...
def plot_coefficient(coefficients, ):
    """
...
def main():
    ...
    result = hill_climbing(lambda coefficient:
                           chi_square(data[:, 1], data[:, 2],
                                      poly_function(data[:, 0], coefficient)))
    ...
    print('Results: \n Reduced chi squared: {0:.3f} \n Coefficient: {1:.2f} +'
          '/- {2:.2f} \n Iterations: {3:d}'.format(result[1] / 49, result[0],
                                                   sigma, result[2]))
